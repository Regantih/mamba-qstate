...
- **5.1 Compounding.** Exp-0 + main: b across bit-widths/lengths. *(figure: KL-vs-step, log-log)*
  Our central observable is the growth exponent b fitting mean KL(full || quant) vs generation length (log-log). The regime is bit-width and horizon dependent. De-risk run (Mamba-2 1.3B, 16 prompts, 512 tokens, exp0_quant_medium): all quantized conditions fit sub-linear (8-bit b in {0.47,0.63,0.56}; 4-bit b in {0.41,0.41,0.39}) but with weak, noisy fits (R^2 0.29-0.66). Strong run (32 prompts, 2048 tokens, exp0_quant_strong) with high-quality fits (R^2 0.85-0.88) shows a clear split: 8-bit quantization is super-linear / COMPOUNDING (b in {1.36,1.47,1.34} for refresh k in {0,16,64}), while 4-bit stays sub-linear (b in {0.45,0.41,0.44}). Periodic full-precision refresh (k in {16,64}) does not reduce b in either regime, so refresh alone does not contain the error. Full-precision controls (>=16-bit) yield KL~0 (b=nan). See paper/figures/exp0_kl_growth.png (512-token) and paper/figures/exp0_kl_growth_strong.png (2048-token). The single-regime hypothesis is too coarse: at a long decode horizon with reliable fits, 8-bit state-quantization error compounds super-linearly whereas 4-bit accumulates sub-linearly; the short-horizon sub-linear reading reflects low statistical power and limited generation length.

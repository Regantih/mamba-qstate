...
### 3.5 Downstream quality: long-context retrieval (NIAH)
Perplexity is corroborated by a needle-in-a-haystack retrieval task: a 4-digit secret code is placed at the start of a ~900-token context (pile filler), and the model must reproduce it after the distractors (20 trials, greedy, seed 1337). Recall accuracy: bits16 20/20 (1.00), bits8 20/20 (1.00), bits4 9/20 (0.45), bits3 11/20 (0.55). 8-bit state quantization is lossless for retrieval, exactly as for perplexity, while 4-bit collapses to chance-like recall as compounding error in the recurrent state corrupts the stored fact. The 3-bit/4-bit ordering is within noise at this sample size. Data: results/exp1/quality.json.

## 4. Discussion and Limitations
Recurrent-state quantization is not analogous to KV-cache quantization below 8-bit: the single overwritten state turns local rounding error into a compounding process, producing a sharp quality cliff. 8-bit is safe; sub-8-bit needs periodic full-precision refresh, which trades a small memory overhead for a return to near-lossless behavior. Limitations: two model scales (1.3b primary, 370m scale check), PTQ only (no QAT), perplexity plus a NIAH retrieval quality axis (3-bit estimate is high-variance), and a Python per-token reference loop rather than a fused kernel.

...
NP = int(os.environ.get("NP","8")); SEQ = int(os.environ.get("SEQ","256"))
...
for bits in [16,8,4,3]:
    p = ppl_for(bits); res["bits%d"%bits]=p; print("bits",bits,"ppl=%.4f"%p, flush=True); torch.cuda.empty_cache()
    json.dump(res, open("/workspace/mamba-qstate/results/exp1/ppl.json","w"), indent=2)

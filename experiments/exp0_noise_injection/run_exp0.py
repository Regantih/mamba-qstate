...
def _policy_factory(noise: float, refresh: int, seed: int):
    return lambda: StatePolicy(
        quant=None,                 # Exp 0 isolates pure noise; quant studied in main sweep
...
def run_cuda(cfg: dict) -> dict:
    ...
    for noise, refresh in conditions:
        ...
        print(f"  done noise={noise} refresh={refresh}: "
              f"b={results[f'noise{noise}_refresh{refresh}']['fit']['exponent_b']:.3f}")
    return results
